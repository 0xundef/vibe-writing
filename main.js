...
var QwenAssistant = class extends OpenAIAssistant {
  constructor(openAIapiKey, qwenApiKey, modelName, maxTokens, baseURL = "https://dashscope.aliyuncs.com/compatible-mode/v1") {
    super(openAIapiKey, modelName, maxTokens);
...
var en = {
  ...
  "status.ready": "Ready",
  // Modal Titles
  "modal.edit-suggestion": "Edit Prompt",
  // Changed from 'Edit Suggestion'
...
function t(key, language = "en", params) {
  var _a2;
  let translation = ((_a2 = translations[language]) == null ? void 0 : _a2[key]) || translations.en[key] || key;
  if (params) {
...
var LanguageManager = class _LanguageManager {
  constructor() {
    this.currentLanguage = "en";
    this.initializeLanguage();
  }
  static getInstance() {
    if (!_LanguageManager.instance) {
      _LanguageManager.instance = new _LanguageManager();
    }
    return _LanguageManager.instance;
  }
  initializeLanguage() {
...
var EditSuggestionModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    saveButton.onclick = async () => {
      ...
      if (!description) {
        new import_obsidian2.Notice(
          translate("validation.description-required") || "Description is required"
        );
        descInput.focus();
        return;
      }
      if (!prompt) {
        new import_obsidian2.Notice(
          translate("validation.prompt-required") || "Prompt is required"
        );
        promptInput.focus();
        return;
      }
...
var ImprovementSuggester = class extends import_obsidian3.SuggestModal {
  ...
  getSuggestions(query) {
    ...
    return this.options.filter(
      (option) => option.name.toLowerCase().includes(query.toLowerCase()) || option.description.toLowerCase().includes(query.toLowerCase())
    );
  }
...
> ${answer.trim().replace(/\n/g, "\n> ")}`;
        ...
        new import_obsidian3.Notice(`AI response added using ${option.name}!`);
        this.plugin.updateStatusBar("Ready");
      } else {
        new import_obsidian3.Notice("Failed to get AI response. Please try again.");
        this.plugin.updateStatusBar("Ready");
      }
    } catch (error) {
      console.error("Error improving text:", error);
      if (error.message === "Request timeout") {
        new import_obsidian3.Notice("AI request timed out. Please try again.");
        this.plugin.updateStatusBar("Timeout");
      } else {
        new import_obsidian3.Notice("Error occurred while improving text.");
...
// src/image-settings.ts
var import_obsidian4 = require("obsidian");
var ImageSettingsManager = class {
  constructor(plugin) {
    this.plugin = plugin;
  }
  /**
   ...
   */
  addImageSettings(containerEl) {
    new import_obsidian4.Setting(containerEl).setName("Image Folder").setDesc("Folder to save compressed images").addText(
      (text) => text.setPlaceholder("AiAssistant/Assets").setValue(this.plugin.settings.imgFolder).onChange(async (value) => {
        this.plugin.settings.imgFolder = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian4.Setting(containerEl).setName("Image Compression Quality").setDesc("Quality for image compression (0.1 - 1.0)").addSlider(
      (slider) => slider.setLimits(0.1, 1, 0.1).setValue(this.plugin.settings.imageCompressionQuality).setDynamicTooltip().onChange(async (value) => {
        this.plugin.settings.imageCompressionQuality = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian4.Setting(containerEl).setName("Image Max Width").setDesc("Maximum width for compressed images").addText(
      (text) => text.setPlaceholder("1920").setValue(this.plugin.settings.imageMaxWidth.toString()).onChange(async (value) => {
    ...
    new import_obsidian4.Setting(containerEl).setName("Image Max Height").setDesc("Maximum height for compressed images").addText(
      (text) => text.setPlaceholder("1080").setValue(this.plugin.settings.imageMaxHeight.toString()).onChange(async (value) => {
        const numValue = parseInt(value);
        if (!isNaN(numValue) && numValue > 0) {
   ...
   */
  addImageSettingsHeader(containerEl) {
    containerEl.createEl("h3", { text: "Image Processing Settings" });
  }
};

// src/settings-tab.ts
var AiAssistantSettingTab = class extends import_obsidian5.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  ...
  display() {
    ...
    new import_obsidian5.Setting(containerEl).setName(translate("settings.model")).setDesc(translate("settings.model-desc")).addDropdown((dropdown) => {
    ...
    new import_obsidian5.Setting(containerEl).setName("Max Tokens").setDesc("Maximum number of tokens to generate").addText(
    ...
    new import_obsidian5.Setting(containerEl).setName("Base URI").setDesc("Base URI for API requests (leave empty for default)").addText(
      (text) => text.setPlaceholder("https://api.openai.com/v1").setValue(this.plugin.settings.qwenBaseURL || "").onChange(async (value) => {
        this.plugin.settings.qwenBaseURL = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian5.Setting(containerEl).setName("API Key").setDesc("API key for the selected model").addText(
      (text) => text.setPlaceholder("Enter your API key").setValue(this.plugin.settings.openAIapiKey || this.plugin.settings.anthropicApiKey || this.plugin.settings.qwenApiKey || "").onChange(async (value) => {
...
var AiAssistantPlugin = class extends import_obsidian6.Plugin {
      ...
      this.addCommand({
        ...
        callback: async () => {
          if (!this.lastSelection) {
            new import_obsidian6.Notice(translate("notice.no-selection"));
            return;
          }
      ...
      this.addCommand({
        ...
        callback: async () => {
          ...
          const editModal = new EditSuggestionModal(
            ...
            void 0,
            // No delete callback for new prompts
      ...
      this.addCommand({
        ...
        callback: async () => {
          ...
          if (!this.lastAiResponse) {
            new import_obsidian6.Notice(translate("notice.no-ai-response"));
            return;
          }
          if (!this.lastSelection.editor) {
            new import_obsidian6.Notice(translate("notice.editor-not-available"));
            return;
          }
          ...
          const newTextEnd = {
            ...
            ch: this.lastAiResponse.split("\n").length === 1 ? this.lastSelection.from.ch + this.lastAiResponse.length : ((_a2 = this.lastAiResponse.split("\n").pop()) == null ? void 0 : _a2.length) || 0
          };
  ...
  async compressImagesInCurrentNote() {
    ...
    if (!file) {
      new import_obsidian6.Notice("No file is currently open.");
      return;
    }
    ...
    if (matches.length === 0) {
      new import_obsidian6.Notice("No images found in the current note.");
      return;
    }
    let compressedCount = 0;
    const totalImages = matches.length;
    const replacements = [];
    new import_obsidian6.Notice(`Found ${totalImages} images. Starting compression...`);
        ...
        img.onload = async () => {
          ...
          canvas.toBlob(
            async (blob2) => {
              ...
              const compressedSize = blob2.size;
              if (compressedSize < originalSize) {
                ...
                const compressedArrayBuffer = await blob2.arrayBuffer();
